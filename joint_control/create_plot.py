...
for i in range(len(act_y.keys())):
    ...
    plt.plot(time[0:len(act_y[act_y.keys()[i]])], act_y[act_y.keys()[i]][0:-1], color="blue", linestyle="-")
    plt.plot(time[0:len(key_y[key_y.keys()[i]])], key_y[key_y.keys()[i]], color="green", linestyle="-")
